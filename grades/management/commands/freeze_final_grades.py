...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):  # pylint: disable=unused-argument
        ...
        if course_id is None:
            raise CommandError('No course_id provided')
        try:
            run = CourseRun.objects.get(edx_course_key=course_id)
        except CourseRun.DoesNotExist:
            raise CommandError('Course Run for course_is "{}" does not exist'.format(course_id))
        freeze_course_run_final_grades.delay(run)
        self.stdout.write(
            self.style.SUCCESS(
                'Successfully submitted async task to freeze final grades for course "{}"'.format(course_id)
            )
        )
